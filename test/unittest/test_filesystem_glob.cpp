...
TEST_CASE("Test Glob with GlobFilesExtended unsupported", "[glob test]") {
	...
	auto hedged_request_filesystem = make_uniq<HedgedFileSystem>(std::move(mock_filesystem), std::move(entry));
	auto results = hedged_request_filesystem->Glob("s3://bucket/snapshots/*.parquet");
